...
import { fire, hashGameState, IllegalMoveError, type GameState } from '../src/game.ts';
...
/**
 * Headroom for the sweeps below.
 *
 * Not cover for a slow sim. These generate hundreds of full-size heightmaps
 * apiece — the tunnelling sweep alone builds 250 — which is several honest
 * seconds on a warm machine and more on a loaded CI box running the rest of
 * this package alongside it. At vitest's 5 s default the longest of them failed
 * intermittently for reasons with nothing to do with the invariant under test,
 * and an invariant test people learn to re-run has stopped being one.
 *
 * The bound is still a bound: any of these taking a minute means something is
 * genuinely wrong.
 */
...
describe('property: same seed → identical outcome', () => {
  it(
    'terrain generation is reproducible',
    () => {
      fc.assert(
  ...
  it(
    'a whole match replays to the same state hash',
    () => {
      fc.assert(
        fc.property(
          seedArb,
          fc.array(fc.record({ angle: angleArb, power: powerArb }), {
            minLength: 1,
            maxLength: 12,
          }),
          (seed, shots) => {
            const play = (): GameState => {
              let state = openedGame({ seed, totalRounds: 3 }, PLAYERS);
...
describe('property: shots never tunnel through terrain', () => {
  it(
    'the point before an impact is always sky',
    () => {
      fc.assert(
  ...
  it(
    'never lands beyond a one-pixel wall it was fired at',
    () => {
      fc.assert(
  ...
  it(
    'stops exactly at the wall whenever it has the range to reach it',
    () => {
      fc.assert(
...
describe('property: health never goes negative', () => {
  it(
    'holds across long random matches with every weapon',
    () => {
      fc.assert(
        fc.property(
          seedArb,
...
describe('property: the sim never throws on valid input', () => {
  it(
    'accepts every in-range angle/power/weapon combination',
    () => {
      fc.assert(
        fc.property(
          seedArb,
  ...
  it(
    'rejects out-of-range input with IllegalMoveError, never a crash',
    () => {
      fc.assert(
        fc.property(
          seedArb,
          ...
          fc.double({ noNaN: false }),
          (seed, angle, power) => {
            const state = openedGame({ seed, totalRounds: 3 }, PLAYERS);
            const shooter = state.tanks[state.activeTank];
            const inRange =
  ...
  it(
    'survives hostile-looking player ids and turn numbers',
    () => {
      fc.assert(
...
describe('property: terrain stays well-formed', () => {
  it(
    'every column stays within the world after any bombardment',
    () => {
      fc.assert(
        fc.property(
          seedArb,
